...
def get_latest_titles(rss_url):
    ...
    try:
        ...
        driver.quit()
        print(page_source[:1000])  # İlk 1000 karakteri terminalde göreceksiniz
        # XML ayrıştırma
...
def get_rss_with_selenium(rss_url):
    options = Options()
    # options.add_argument("--headless")  # Tarayıcıyı arka planda açar
    options.add_argument("--disable-gpu")
    options.add_argument("--no-sandbox")
    driver = webdriver.Chrome(service=Service(ChromeDriverManager().install()), options=options)
    driver.get(rss_url)
